...
- 滤波在 **GPU** 上执行（`comfy.model_management.get_torch_device()`）；输入帧先搬到 GPU，处理完搬回输入所在设备，下游行为不变。
- 内部按 **8 帧一块**滑窗处理，每块只带前后各 1 帧上下文重叠。整段视频不会连同邻居副本/权重张量一起常驻显存或系统内存。
- 早期版本在 CPU 内存中对整段帧序列做双边滤波，长视频（如 4K 帧序列）会耗尽系统内存；GPU 分块后系统内存峰值与输入规模解耦。
